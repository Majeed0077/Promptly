...
type ApiAdminPrompt = Partial<
  Omit<AdminPrompt, "id" | "tags" | "status">
> & {
  id?: unknown;
  _id?: unknown;
  tags?: unknown;
  ...
  useEffect(() => {
    ...
    const loadPrompts = async () => {
      try {
        ...
        if (!isMounted) return;
        const mapped = (data.prompts as ApiAdminPrompt[]).map((prompt) =>
          mapPrompt(prompt)
        ) as AdminPrompt[];
        setPrompts(mapped);
      } catch {
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-white/55">
                Create, review, and organize reusable prompts.
              </p>
            </div>
            <AdminUserActions />
          </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="relative">
                  <select
                    ...
                    className="appearance-none rounded-lg border border-white/10 bg-white/[0.04] px-4 py-2.5 pr-9 text-xs font-semibold text-white/76 transition hover:border-white/20 focus:outline-none"
                  >
                ...
                <button
                  onClick={handleOpenCreate}
                  className="flex items-center gap-2 rounded-lg border border-[rgba(216,181,109,0.38)] bg-[var(--accent)] px-4 py-2.5 text-xs font-semibold text-[#17130b] transition hover:bg-[var(--accent-strong)]"
                >
                  <PlusIcon className="h-4 w-4" />
                  New Prompt
                </button>
...
function mapPrompt(prompt: ApiAdminPrompt): AdminPrompt {
  const status =
    prompt.status === "Draft" ||
  ...
  return {
    ...
    imageUrl: String(prompt.imageUrl ?? ""),
    status,
  };
}
